...
For each $1\le k \le N$ let $X_k(t)$ be an independent Markov chain on $\mathcal{A}$ with initial state $X_k(0)=X_k$,
and suppose that the Markov chain has unique stationary distribution $\pi$.
...
a.  Design a Markov chain whose stationary distribution is $\pi_\alpha$.
...
Any insertions present then has probability $1/2$ of being passed on to the chromosome inherited by the offspring.
...
a. Simulate the process, beginning with a single copy of the transposon at one location. 
    ...
    * even so, there are very few locations in the genome at which an insertion is common.
...
The probability that it has $k$ offspring is messy, but the mean number of offspring is $2 \times ( \mu + (1-\mu)/2 ) = 1+2\mu$,
so the mean total number of copies is $(1+2\mu)^t$.
...
this has generating function $\phi(u) = \exp((1-\mu)(u-1))$.
...
The total number of non-transposon base pairs is $3 \times 10^7 \times 10^9 = 10^{16}$;
Using *b*, the total number of transposon base pairs is $(1+2\mu)^t \times 3 \times 10^3 = 3 \times 10^{3+2 t \times 10^{-2} /\log(10)}$;
this equals $3 \times 10^{15}$ at $t=600\log(10)=1382$ generations.
